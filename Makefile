...
CONF=$(DESTDIR)/etc/linuxmuster-client/tor-messenger
BIN=$(DESTDIR)/usr/bin
SBIN=$(DESTDIR)/usr/sbin
SHARE=$(DESTDIR)/usr/share/linuxmuster-client-tor-messenger
...
install:
	...
	@install -oroot -groot --mode=0775 scripts/linuxmuster-client-tor-messenger-downloader $(SBIN)
	@echo '   * Installing starter script'
	@install -d -m755 -oroot -groot $(BIN)
	@install -oroot -groot --mode=0775 scripts/linuxmuster-client-tor-messenger $(BIN)
	@echo '   * Installing unity dash entry'
	@install -d -m0755 -oroot -groot $(DESKTOP)
	@install -oroot -groot --mode=0644 icons/tor-messenger.desktop $(DESKTOP)
	@echo '   * Installing icon'
	@install -d -m0755 -oroot -groot $(ICON)
	@install -oroot -groot --mode=0644 icons/tor-messenger.png $(ICON)
